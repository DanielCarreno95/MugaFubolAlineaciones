...
EXCEL_DEFAULT = APP_DIR / "Fulbichi.xlsx"
...
def _sync_players_csv_revision() -> int:
    """Si el CSV cambió en disco (p. ej. push a GitHub), fuerza refresco del data_editor."""
    sig = dm.players_csv_signature(DATA_DIR)
    if st.session_state.get("players_csv_sig") != sig:
        st.session_state["players_csv_sig"] = sig
        st.session_state["players_editor_rev"] = int(st.session_state.get("players_editor_rev", 0)) + 1
    return int(st.session_state.setdefault("players_editor_rev", 0))


def _clear_players_editor_state() -> None:
    prefix = "players_editor"
    for key in list(st.session_state.keys()):
        if key == prefix or key.startswith(f"{prefix}_"):
            del st.session_state[key]


def _reload_players_from_csv() -> None:
    _clear_players_editor_state()
    st.session_state["players_csv_sig"] = dm.players_csv_signature(DATA_DIR)
    st.session_state["players_editor_rev"] = int(st.session_state.get("players_editor_rev", 0)) + 1
    st.session_state["has_generated"] = False
    st.session_state["result_opt1"] = None
    st.session_state["result_opt2"] = None
...
def _whatsapp_teams_text(team_a: Tuple[dm.Player, ...], team_b: Tuple[dm.Player, ...]) -> str:
    lines = ["Equipo A:"]
    lines.extend(p.name for p in team_a)
    lines.append("")
    lines.append("Equipo B:")
    lines.extend(p.name for p in team_b)
    return "\n".join(lines)
...
excel_path = st.sidebar.text_input(
    "Excel (solo para reinicializar CSV)",
    value=str(EXCEL_DEFAULT.resolve()),
    help="No es la fuente de datos del día a día. Los jugadores se leen de data/players.csv.",
)
...
editor_rev = _sync_players_csv_revision()

st.sidebar.caption("**Fuente de jugadores (la que usa la app):**")
st.sidebar.code(str(PLAYERS_CSV.resolve()), language=None)
if PLAYERS_CSV.exists():
    n_players = len(players_df)
    st.sidebar.caption(f"{n_players} jugadores cargados desde el CSV.")
else:
    st.sidebar.warning("Aún no existe `data/players.csv`. Se creará al guardar o desde el Excel.")

if st.sidebar.button("Recargar jugadores desde CSV", use_container_width=True):
    players_df = dm.load_players_csv(DATA_DIR)
    _reload_players_from_csv()
    st.sidebar.success("CSV recargado.")
    st.rerun()
...
with tab2:
    ...
    st.caption(
        "Los jugadores se leen de **`data/players.csv`**, no del Excel. "
        "Si actualizaste el CSV en GitHub y no ves cambios, pulsa **Recargar jugadores desde CSV** en la barra lateral."
    )
    ...
    edited = st.data_editor(
        ...
        num_rows="dynamic",
        key=f"players_editor_{editor_rev}",
        column_config={
...
with tab1:
    ...
    def _render_option(title: str, res: TeamResult) -> None:
        ...
        st.divider()
        st.subheader("Formato WhatsApp (copiar y pegar)")
        st.text_area(
            "Lista de equipos",
            value=_whatsapp_teams_text(tuple(res.team_a), tuple(res.team_b)),
            height=280,
            key=f"whatsapp_text_{title}",
            help="Selecciona y copia este texto para compartirlo por WhatsApp.",
        )
