...
def load_players_csv(data_dir: Path) -> pd.DataFrame:
    """Lee `players.csv` del disco (fuente de verdad de la app)."""
    ensure_data_dir(data_dir)
    p = config_paths(data_dir)["players_csv"]
    if not p.exists():
        return _normalize_players_df(
            pd.DataFrame(columns=["name", "football", "physical", "pos1", "pos2"])
        )
    df = pd.read_csv(p, encoding="utf-8")
    return _normalize_players_df(df)


def players_csv_signature(data_dir: Path) -> str:
    """Identificador del CSV en disco (mtime + tamaño) para detectar cambios externos."""
    p = config_paths(data_dir)["players_csv"]
    if not p.exists():
        return "missing"
    st = p.stat()
...
def load_or_init_players(data_dir: Path, excel_path: Optional[Path] = None) -> pd.DataFrame:
    ...
    p = config_paths(data_dir)["players_csv"]
    if p.exists():
        return load_players_csv(data_dir)
